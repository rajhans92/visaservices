...
class ServicesController extends Controller
{
    ...
    public function pages(Request $request)
    {
      ...
      foreach ($tempServicesTable as $key => $value) {
        ...
        $allServicesData[strtolower($value->visa_country_name)]['USD']['standard'] = number_format($value->standard_usd_price,2);
        $allServicesData[strtolower($value->visa_country_name)]['USD']['rush'] = number_format($value->rush_usd_price,2);
        $allServicesData[strtolower($value->visa_country_name)]['USD']['express'] = number_format($value->express_usd_price,2);

        foreach ($currencyRate as $key2 => $value2) {

          $allServicesData[strtolower($value->visa_country_name)][strtoupper($value2->code)]['standard'] = number_format($value2->rate * $value->standard_usd_price,2);

          $allServicesData[strtolower($value->visa_country_name)][strtoupper($value2->code)]['rush'] = number_format($value2->rate * $value->rush_usd_price,2);

          $allServicesData[strtolower($value->visa_country_name)][strtoupper($value2->code)]['express'] = number_format($value2->rate * $value->express_usd_price,2);
    ...
    public function serviceContactUs(Request $request){
        ...
        DB::table('service_contact_us')->insert([
            ...
            'message' => $request['msg'],
            'visa_country' => strtolower($request['country']),
